...
class inferThread(threading.Thread):
    ...
    def run(self):
        ...
        #set resolution of video output
        video_out_frame_width = int(self.cap.get(3))
        video_out_frame_height = int(self.cap.get(4))
        video_out_fps = int(self.cap.get(5))

        #video_out_size = (video_out_frame_width, video_out_frame_height)

        # Below VideoWriter object will create
        # a frame of above defined The output 
        # is stored with the specific format
        video_path = OUTPUT_FOLDER + '/raspberry_cam_from_drone_' + datetime.now().strftime("%Y-%m-%d_%H.%M.%S") + '.avi'
        #video_path = 'raspberry_cam_from_drone.avi'
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        print('Video name:', video_path, '. Frame size: width:', video_out_frame_width, ', height:', video_out_frame_height, ', FPS:',video_out_fps)
        video_out = cv2.VideoWriter(video_path, 
                         fourcc,
                         float(17), 
                         (video_out_frame_width,video_out_frame_height),
                         True)
        ...
        while True:
            ...
            print("inference: time->{:.2f}ms, fps: {:.2f}, total fps: {:.2f}, frame obj: {}, total obj: {}".format(use_time * 1000, 1 / (use_time), fps, number_of_objects, total_number_of_objects))
            # Inference FPS:
            ...
            if run_in_foreground:
                ...
                cv2.resizeWindow('Drone camera',800,800)
            	#cv2.moveWindow('Drone camera',40,30)
                cv2.imshow("Drone camera", result)
                
            if first_run == False:
                firt_run = True
                #Identify when process is running in background
                pid = os.getpid()
                output = subprocess.check_output(["ps", "-o", "stat", "-p", str(pid)])
                lines = output.strip().decode('utf-8')
                if "+" in lines:
                    run_in_foreground = True
                    print('Running python script in foreground.')
                    #cv2.imshow("Drone camera", result)
                    #log_file = LOG_FOLDER + '/python_script_running_' + datetime.now().strftime("%Y-%m-%d_%H.%M.%S") + '.txt'
                    #with open(log_file, "a") as f:
                    #    f.write("Running in background mode!, with PID: "+ str(pid))
                    #    f.close()                    
...
if __name__ == "__main__":
    ...
    PLUGIN_LIBRARY = "generated_plugins/libmyplugins.so"
    engine_file_path = "generated_engines/yolov7-tiny.engine"
